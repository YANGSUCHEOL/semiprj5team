...
public class QnaDao {
	...
	public List<QuestionVo> selectList(Connection conn, PageVo pv) {
		String sql = "SELECT * FROM ( SELECT ROWNUM AS RNUM , T.* FROM ( SELECT Q.NO ,Q.M_NO ,Q.TITLE ,Q.CONTENT ,Q.HIT ,Q.ENROLL_DATE ,Q.UPDATE_DATE ,Q.DELETE_YN ,Q.ANSWER_YN ,M.NICK AS WRITER FROM QUESTION Q JOIN MEMBER M ON Q.M_NO = M.NO WHERE Q.DELETE_YN = 'N' ORDER BY B.NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
