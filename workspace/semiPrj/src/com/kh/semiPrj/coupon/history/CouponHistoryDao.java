...
public class CouponHistoryDao {
	...
	public List<CouponHistoryVo> couponList(Connection conn, String mno) {
		...
		String sql = "SELECT H.NO , R.NAME AS RESTAURANT , C.NO AS COUPON , M.NO AS NAME , H.USED_YN , H.ENROLL_DATE FROM COU_HISTORY H JOIN RESTAURANT R ON H.R_NO = R.NO JOIN COUPON C ON H.C_NO = C.NO JOIN MEMBER M ON H.M_NO = M.NO WHERE H.M_NO = ?";
