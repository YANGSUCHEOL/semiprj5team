...
#ans-expect{
	...
	<div id="background">
	    ...
	    <div id = "main">
	        ...
	        <div id="list">
	            ...
	            <%for(int i = 0; i < bvoList.size(); i++) { %>
	            	<div><%= bvoList.get(i).getNo() %></div> <!-- 타이틀은 제대로 가져와지는데 글번호는 뭔데 -->
	            	<div><a href="/semiPrj/bqna/detail?no=<%= bvoList.get(i).getNo() %>"><%= bvoList.get(i).getTitle() %></a></div>
	           		<!-- customer 뺌 -->
	           		<div><%= bvoList.get(i).getbNo() %></div>
	           		<div><%= bvoList.get(i).getEnrollDate() %></div>
	           		<% if(bvoList.get(i).getAnswerYn().equals("Y")) {%>
	    ...
	    <div id="page-area" align="center">
	           	 	<a href="/semiPrj/bqna/write"><button>작성하기</button></a>
